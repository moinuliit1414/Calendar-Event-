...
var ListViewGrid = Grid.extend({
	...
	renderEmptyMessage: function() {
		// this.el.html(
			...
			// '</div>'
		// );
		var startfrom=this.startMs;
		this.endMs
		var tableEl = $('<table class="fc-list-table ' + this.view.calendar.theme.getClass('tableList') + '"><tbody/></table>');
		var tbodyEl = tableEl.find('tbody');
		
		while(startfrom<this.end){
			
			startfrom.setDate(startfrom.getDate() + 1);
		}
		// for (dayIndex = 0; dayIndex < this.dayDates; dayIndex++) {
			
			// tbodyEl.append(this.dayHeaderHtml(this.dayDates[dayIndex]));
			// console.log('hsviab aogfiauwb');
		// }
	...
	renderSegList: function(allSegs) {
		...
		for (dayIndex = 0; dayIndex < this.dayDates.length; dayIndex++) {
			tbodyEl.append(this.dayHeaderHtml(this.dayDates[dayIndex]));
			daySegs = segsByDay[dayIndex];
			if (daySegs) { // sparse array, so might be undefined
	...
	groupSegsByDay: function(segs) {
		...
		//console.log('segsByDay print...' +JSON.stringify(segsByDay));
		return segsByDay;
	},
